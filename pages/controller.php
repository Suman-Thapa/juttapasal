...
session_start();
    ...
    if(!$connection) {
        die(json_encode(array("success" => false, "error" => "DB connection failed")));
    }
    ...
    function manage_users($connection) {
        ...
        while($row=mysqli_fetch_array($result)){
            ...
            $email = $row["Email"];
            $role = $row["Role"];
            $data[] = array("user_id"=>$user_id, "last_name"=>$last_name, "first_name"=>$first_name, "address"=>$address, "date_of_birth"=>$date_of_birth, "gender"=>$gender, "email"=>$email, "role"=>$role);
        }
    ...
    // NOTE: check_out()/place_order()/change_phone_number()/change_address() below
    // are superseded by pages/checkout.php + server/place_order_cod.php +
    // server/payment_esewa.php + server/payment_khalti.php, which capture
    // delivery details per-order (matching the `orders` table) instead of
    // mutating the Users row. Left here only so nothing else that might
    // reference these AJAX actions breaks; the new checkout page doesn't call them.
    ...
    function login($connection) {
        if (isset($_POST["username"]) && isset($_POST["password"])) {
            ...
            $query = "SELECT ID, Password, Role FROM Users WHERE Email='$username'";
            $result = mysqli_query($connection, $query);
            ...
            // Supports both password_hash()'d passwords (new signups) and any
            // legacy plaintext rows created before this fix.
            $valid = password_verify($password, $row["Password"]) || $password === $row["Password"];

            if ($valid) {
                $_SESSION["user_id"] = $row["ID"];
                $_SESSION["role"]    = $row["Role"] ?? 'user';
                return array("success"=>true, "role"=>$_SESSION["role"]);
            }
            return array("success"=>false);
        } else {
            return array("success"=>false);
    ...
    function register($connection) {   
        ...
        if ($row_email_check["count"]==0) {                
            ...
            $query = "INSERT INTO Users (Last_Name, First_Name, Address, Date_of_Birth, Gender, Email, Password)
        ...
        if(isset($_SESSION['user_id'])){
            ...
            if ($quantity >= 1) {
                if (is_added($connection, $product_id, "cart_item")) {
                    ...
                    $result = mysqli_multi_query($connection, $query);
                    if(!$result)
                        return array("success" => false, "reason"=>"error", "message"=>"failed to add product");
                    return array("success" => true);
                } else {
                ...
                return array("success" => false, "reason"=>"out_of_stock");
            }
        }
        return array("success" => false, "reason"=>"not_logged_in");
    }
    ...
    function get_user_info($connection){
        if(isset($_SESSION["user_id"])){
            $user_id = $_SESSION["user_id"];
            $query = "SELECT Users.First_Name, cart_item.Item_quantity AS quantity FROM cart_item INNER JOIN Users ON cart_item.User_id =  Users.ID WHERE User_id = '$user_id'";
            $result = mysqli_query($connection, $query);
            $item_count = 0;
            $username = "";
            ...
            if($result){
                ...
                if($item_count == 0){
                    $query_username = "SELECT First_Name AS fname FROM Users WHERE ID = '$user_id'";
